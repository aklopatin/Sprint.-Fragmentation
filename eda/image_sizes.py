...
from .data import get_pairs, get_image_sizes


def run_image_sizes(root="train_dataset_for_students"):
    """Считает и выводит статистику размеров, строит гистограммы и scatter."""
    root = Path(root)
    ...
    axes[1, 1].axvline(
        aspect_train.mean(), color="red", linestyle="--", label=f"mean={aspect_train.mean():.2f}"
    )
